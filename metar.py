...
def fetch_winds_aloft(icao):
    resp = requests.get(WINDTEMP_URL, params={"ids": icao, "format": "json", "level": "lo"}, timeout=10)
    resp.raise_for_status()
    if not resp.content:
        return None
    data = resp.json()
...
def render_winds_aloft(data):
    ...
    t = Text()
    tbl = Table(box=None, show_header=True, padding=(0, 1), expand=False)
    tbl.add_column("Altitude", style="dim")
    tbl.add_column("Wind", style="cyan")
    tbl.add_column("Speed", style="cyan")
    tbl.add_column("Temp", style="yellow")

    for profile in profiles:
        alti_ft = profile.get("altim_ft_msl")
        wdir = profile.get("wdir")
        wspd = profile.get("wspd")
        temp = profile.get("temp")

        alt_str = f"{alti_ft:,}ft" if alti_ft else "—"
        wind_str = f"{int(wdir)}°" if wdir is not None else "—"
        speed_str = f"{int(wspd)}kt" if wspd is not None else "—"
        temp_str = f"{int(temp)}°C" if temp is not None else "—"

        speed_style = "bold cyan" if wspd and wspd > 30 else "cyan"
        tbl.add_row(alt_str, wind_str, Text(speed_str, style=speed_style), temp_str)
...
def show_station(icao, show_taf=False, raw_only=False, show_sigmet=False, show_pireps=False, show_aloft=False, crosswind_runway=None):
    ...
    if show_aloft:
        ...
        try:
            ...
            panel = render_winds_aloft(aloft)
            if panel:
                console.print(panel)
            else:
                console.print(Panel(
                    Text("no winds aloft data available", style="dim yellow"),
                    border_style="yellow",
                ))
        except requests.RequestException:
            console.print(Panel(
...
def main():
    ...
    parser.add_argument("--aloft", action="store_true", help="Include winds aloft data")
